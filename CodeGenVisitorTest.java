...
public class CodeGenVisitorTest implements ImageResources{
	...
	public void imageGenGreen() throws Exception{
		...
		String[] commandLineArgs = {}; 
		runCode(prog, bytecode, commandLineArgs);		
		BufferedImage imageRef = ImageSupport.makeConstantImage(0xFF000, 256, 256);
		BufferedImage image = RuntimeLog.globalImageLog.get(0);
		ImageSupport.compareImages(imageRef, image);
...
@Test
	public void image7() throws Exception{
		...
		String input = prog
				...
				+"\nh -> f;"
				;
				//. expected:<9281988> but was:<4206890>
		byte[] bytecode = genCode(input);		
		...
		for(int y = 0; y < 1024; y++) {
			for (int x = 0; x < 1024; x++) {
				...
				int pixelRef = ImageSupport.getPixel(loggedImage1, x, y);
				int pixel = ImageSupport.getPixel(loggedImage, RuntimeFunctions.cart_x(r, a),RuntimeFunctions.cart_y(r, a));
...
//image8: Testing failed with input: "image8//args: <imageURL>\nimage[1024,1024] g; \n\nimage[1024,1024] h; \ng <- @ 0;\ng -> SCREEN;\nh[[x,y]] =  Z-g[x,y];h -> SCREEN; \n". expected:<9281988> but was:<9413574>
//image9: Testing failed with input: "image9//args: <imageURL>\nimage[1024,1024] g; \n\nimage[1024,1024] h; \ng <- @ 0;\ng -> SCREEN;\nh[[x,y]] =  (g[x,y] > Z/2) ? Z-g[x,y] : g[x,y];h -> SCREEN; \n". expected:<7363641> but was:<7495227>
...
public void image8() throws Exception {
	...
	for(int y = 0; y < 1024; y++) {
		for (int x = 0; x < 1024; x++) { 
			...
			assertEquals(Z -pixelRef & 0x00FFFFFF, pixel);
		}
	}
...
@Test
public void progFuncAbs() throws Exception {
	//scan, parse, and type check the program
	...
	assertEquals("-1;3;",RuntimeLog.globalLog.toString());
}

@Test
public void prog5c() throws Exception {
	//scan, parse, and type check the program
	String prog = "prog5c";
	String input = prog
			+"\nint v1;"
	...
	runCode(prog, bytecode, commandLineArgs);	
	show("Log:\n"+RuntimeLog.globalLog);
	assertEquals("8;",RuntimeLog.globalLog.toString());
}
